...
:END:
:LOGBOOK:
- Note taken on [2026-03-13 Fri 20:14] \\
  Changed from rye to uv
:END:
...
- I will run the tests again, but this time with the ~--overwrite~ flag, and also taking advantage of parallel execution with ~-n=auto~ (using pydist-xdist ~rye add pytest-xdist --dev~)
  #+begin_src sh :dir ../notebooks :results output verbatim
    uv run pytest --nbmake --overwrite -n=auto demo*.ipynb
  #+end_src
...
- Check that the tests pass
  ...
  - Unit tests:
    #+begin_src sh
      uv run pytest
    #+end_src
    *still need to write these tests*
- Regenerate the pure python versions of the notebooks
  #+begin_src sh
    uv run jupytext --show-changes --update --sync *.py
  #+end_src
- Buld and test that the version is updated dynamically in python code
  #+begin_src sh
    rm -rf dist && uv build
    uv run --with confitti --no-project -- python -c "import confitti; print(confitti.__version__)"
  #+end_src
- Add description of changes to [[file:../CHANGELOG.md]]
- Update [[file:../README.md]] if necessary (for instance, the "What's new" section)
- Make a git commit and add a git tag
  #+begin_src sh
    git commit -am "Release v$(uv version --short)" && git tag "v$(uv version --short)"
  #+end_src
  - Or use magit
- Publish to PyPI
#+begin_src sh
